...
export default function ProductCardAr({ product }: { product: Product }) {
  ...
  const handleAddToCart = () => {
    ...
    // addItem بيفتح درج السلة تلقائياً، مفيش تحويل مباشر للشيك أوت
  };
        ...
        <div style={{ marginBottom: "0" }}>
          {!showOptions ? (
            ...
            <div className="flex flex-col gap-0 font-bold">
              <SelectField value={selectedSize} onChange={setSelectedSize} options={sizes} position="right" />
              <SelectField value={selectedColor} onChange={setSelectedColor} options={colors} position="left" />
            </div>
          )}
        </div>
